...
You can select desired plot by `which` option as the same manner as standard `plot`. Following shows the comparison between `ggfortify` and default.
...
```{r, fig.height = 4}
par(mfrow = c(1, 2))
m <- glm(Petal.Width ~ Petal.Length, data = iris, family='poisson')
